...
const FAQItem = ({ items, onClick, isOpen }) => {
  ...
  return (
    <div
      ...
      id={`faq-item-${items.id}`}
    >
      <button
        className="w-full py-5 px-4 flex justify-between items-center text-left focus:outline-none rounded-lg transition-all duration-200 cursor-pointer text-gray-900 dark:text-white hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r hover:from-indigo-600 hover:to-purple-600 dark:hover:from-indigo-400 dark:hover:to-purple-400"
        onClick={() => onClick(items.id)}
      >
        <span className="text-lg font-medium pr-6">{items.questions}</span>
